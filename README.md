...
session. The Python template also resolves and commits `uv.lock` before Git
initialization.
...
The tests generate Python CLI and library projects plus Rust single-crate and
workspace binary and library projects in temporary directories. They check
conditional files and Git state, record GitHub CLI calls with a local fake, and
run each language's formatting, linting, type-checking, testing, and build
